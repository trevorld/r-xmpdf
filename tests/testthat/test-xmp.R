...
test_that("get_xmp() / set_xmp()", {
	...
	x <- xmp(
		alt_text = "An alternative image text",
	...
	expect_equal(x$get_item("dc:contributor"), "An updated contributor")
	expect_equal(x$get_item("Iptc4xmpCore:Location"), "A sublocation (legacy)")
	...
	expect_equal(x2$get_item("dc:contributor"), "An updated contributor")
	expect_equal(x2$get_item("Iptc4xmpCore:Location"), "A sublocation (legacy)")
...
test_that("get_xmp_exiftool() / set_xmp_exiftool()", {
	...
	x$title <- as_lang_alt(
		c("en" = "An English Title", "fr" = "A French Title"),
		default_lang = "en"
	)
