...
Unofficially if you read the `exiftool` [XMP Tags](https://exiftool.org/TagNames/XMP.html) and [Structured Information](https://exiftool.org/struct.html) documentation you may be able to set a "flattened" tag to set or manually construct a serialized struct string for a non-"flattened" tag that you could then set via an `xmp()` objects `set_item()` method and it may work (for now) when using `set_xmp_exiftool()`.
