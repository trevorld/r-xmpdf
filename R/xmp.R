...
xmp <- function(
	...
	Xmp$new(
		...
		attribution_url = attribution_url, #cc
		license = license,
		more_permissions = more_permissions,
		alt_text = alt_text,
		ext_description = ext_description, # Iptc4xmpCore
		credit = credit,
		date_created = date_created,
		headline = headline, # photoshop
		create_date = create_date,
		creator_tool = creator_tool,
		modify_date = modify_date, # xmp
		marked = marked,
		usage_terms = usage_terms,
		web_statement = web_statement, # xmpRights
		spdx_id = spdx_id,
		auto_xmp = auto_xmp
	)
}
...
Xmp <- R6Class(
	...
	public = list(
		...
		print = function(
			mode = c("null_omit", "google_images", "creative_commons", "all"),
			xmp_only = FALSE
		) {
			mode <- match.arg(mode)
			text <- character(0)
			...
			for (key in tags) {
				...
				if (is.null(value) && mode == "null_omit") {
					next
				}
			...
			if (length(text)) {
				...
				if (!xmp_only) {
					if (length(self$auto_xmp)) {
						text <- c(
							stri_join("i  auto_xmp (not XMP tag) := ", x_format(self$auto_xmp)),
							text
						)
					}
					if (!is.null(self$spdx_id)) {
						text <- c(
							stri_join("i  spdx_id (not XMP tag) := ", x_format(self$spdx_id)),
							text
						)
					}
				}
		...
		set_item = function(key, value) {
			...
			if (lkey %in% c("attribution_name", "attributionname", "cc:attributionname")) {
				...
				lkey %in% c("alt_text", "alttextaccessibility", "iptc4xmpcore:alttextaccessibility")
			) {
				self$alt_text <- value
			} else if (
				lkey %in%
	...
	active = list(
		...
		attribution_url = function(value) {
			private$active_helper("attribution_url", value, as_url_value)
		},
		creator = function(value) private$active_helper("creator", value),
		create_date = function(value) {
			private$active_helper("create_date", value, as_datetime_value)
		},
		...
		date_created = function(value) {
			private$active_helper("date_created", value, as_datetime_value)
		},
		description = function(value) private$active_helper("description", value, as_la_value),
		ext_description = function(value) {
			private$active_helper("ext_description", value, as_la_value)
		},
		...
		modify_date = function(value) {
			private$active_helper("modify_date", value, as_datetime_value)
		},
		more_permissions = function(value) {
			private$active_helper("more_permissions", value, as_url_value)
		},
	...
	private = list(
		...
		auto_rights = function() {
			...
			if (!is.null(private$tags$creator) && !is.null(private$tags$date_created)) {
				...
				year <- datetimeoffset::format_iso8601(
					private$tags$date_created,
					precision = "year"
				)
				if (is.null(self$spdx_id)) {
		...
		auto_usage_terms = function() {
			...
			if (!is.null(private$tags$spdx_id)) {
				...
				value <- stri_join(
					"This work is licensed to the public under the ",
					name,
					" license ",
					url
				)
