...
need_to_install_str <- function(fn_call) {
	c(
		"!" = sprintf(
			"You must install (only) one suggested R package or system command to use %s",
			sQuote(fn_call)
		)
	)
}
...
install_package_str <- function(package) {
	c(
		x = sprintf("The suggested package %s is not installed", sQuote(package)),
...
install_pdftk_str <- function() {
	c(
		...
		i = paste(
			sQuote('Sys.setenv(PDFTK_PATH = "/path/to/pdftk")'),
...
install_gs_str <- function() {
	c(
		...
		i = paste(
			sQuote('Sys.setenv(R_GSCMD = "/path/to/gs")'),
			"if installed but not detected on PATH"
		)
	)
}
...
install_exiftool_str <- function() {
	c(
		...
		i = paste(
			sQuote('Sys.setenv(ET_EXIFTOOL_PATH = "/path/to/exiftool")'),
			"if installed but not detected on PATH"
		)
	)
}
