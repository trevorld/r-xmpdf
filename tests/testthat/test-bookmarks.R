...
test_that("set_bookmarks", {
	...
	bookmarks <- data.frame(title = "Page 2", page = c(2L))
	set_bookmarks(bookmarks, f1, f2)
...
test_that("`get_count()` and `get_level()`", {
	...
	expect_equal(
		get_count(c(1, 2, 3, 2, 3, 1, 2), c(TRUE, FALSE, NA, TRUE, NA, FALSE, NA)),
		c(2, -1, 0, 1, 0, -1, 0)
	)
...
test_that("`cat_bookmarks()` works", {
	...
	expect_equal(bm$title, c("Page 1", "Page 2", "Page 1", "Page 2"))
	...
	expect_equal(bm$title, c(basename(f2), "Page 1", "Page 2", basename(f3), "Page 1", "Page 2"))
	...
	expect_equal(
		bm$title,
		c("R Graphics Output", "Page 1", "Page 2", "R Graphics Output", "Page 1", "Page 2")
	)
})
