...
if (TRUE) {
	...
	spdx_licenses <- mutate(
		...
		pd = ifelse(id %in% c("MIT"), FALSE, pd)
	)
	which_cc <- grep("^Creative Commons", spdx_licenses$name)
	spdx_licenses$name[which_cc] <- gsub(
		" Non Commercial",
		"-NonCommercial",
		spdx_licenses$name[which_cc]
	)
	spdx_licenses$name[which_cc] <- gsub(
		" Share Alike",
		"-ShareAlike",
	...
	spdx_licenses <- select(spdx_licenses, id, name, url, fsf, osi, deprecated, url_alt, pd) |>
		as.data.frame()
